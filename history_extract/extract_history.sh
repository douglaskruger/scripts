...
-- create the table but do not include the row
select 0 as "ABB_NM_IRN",0 as "ObjectId" into abb_mapping where 1=2
go
...
{
        echo "Inserting the ABB_Mapping data into the database"
...
depending on the dataset available, can generate GB of data files. The default is ${BCP_LINES} rows per file.
        ...
        -all                    Enables the -create_sql, create_abb, insert_abb, and extract flags
        -create_sql             Creates the supporting tables and views
        -create_abb             Creates the abb supporting tables and views
        -insert_abb             Insert the abb mapping data
...
if [ ${ALL} -eq 1 ]; then
        export CREATE_SQL=1
        export CREATE_ABB_IN_DB=1
        export INSERT_ABB_IN_DB=1
        export EXTRACT_HISTORY=1
fi
